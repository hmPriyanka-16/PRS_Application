...
namespace PRSwebapp
{
    public partial class hold : System.Web.UI.Page
    {
        ...
        bool isActionColumnVisible = false;
    ...
    ORDER BY UploadDate DESC";
        ...
        protected void gvMainPending_RowCommand(object sender, System.Web.UI.WebControls.GridViewCommandEventArgs e)
        {
            ...
            else if (e.CommandName == "Resume")
            {
                ...
                UpdatePRSStatus(prsNo, "Resume", remarks);

                BindInProgressTasks(); // refresh grid
            }
            // -------------------------
        ...
        protected void gvMainPending_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                Label Lbl_l_act = (Label)e.Row.FindControl("Lbl_last_action");

                //if (btnResume == null) return;

                string userRole = Session["Role"]?.ToString();
                string userDept = Session["department"]?.ToString();
                //btnResume.Text = userRole + Lbl_l_act.Text;

                btnResume.Visible = true;
                isActionColumnVisible = true;
                if (userRole!= Lbl_l_act.Text )
               {
                   btnResume.Visible = false;
                    isActionColumnVisible = false;
                }
               /*
               string rowDept = DataBinder.Eval(e.Row.DataItem, "Department")?.ToString();
               string holdBy = DataBinder.Eval(e.Row.DataItem, "LastAction")?.ToString();

               // 🔥 CONDITION: match BOTH role + department
               bool canResume = false;

               if (!string.IsNullOrEmpty(userRole) && !string.IsNullOrEmpty(userDept))
               {
                   if (userDept == rowDept && holdBy == userRole)
                   {
                       canResume = true;
                   }
               }
    ...
    private void UpdatePRSStatus(string prsNo, string status, string comments = null)
        ...
        protected void gvMainPending_DataBound(object sender, EventArgs e)
        {
            if (!isActionColumnVisible)
            {
                gvMainPending.Columns[8].Visible = false; // Action column index
            }
        }
