...
namespace PRSwebapp
{
    ...
    ORDER BY UploadDate DESC";
        ...
        myModal.show();";
            ...
            else if (e.CommandName == "OpenPRS")
            {
                string prsNo = e.CommandArgument.ToString();

                // 🔥 Redirect to your PRS form page
                Response.Redirect("queryans.aspx?prsNo=" + prsNo);
            }
            // -------------------------
            // VIEW DOCUMENTS
            // -------------------------
...
Union All
    Select 1 TypeID,SP.PONumber,RF.FileName,
        ...
        protected void gvMainPending_DataBound(object sender, EventArgs e)
        {
            ...
            {
                gvMainPending.Columns[8].Visible = false; // Action column
            }
        }
        protected void gvMainPending_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                Label Lbl_l_act = (Label)e.Row.FindControl("Lbl_l_roleID");
